...
import { GitDiffAdapter, resolveLocalRef, runGit } from '@semantic-diff-tracer/diff-git';
import {
  OctokitPrAdapter,
  parseGithubPrUrl,
  parseNumberOnly,
...
/**
 * Compose the Claude-backed skill deps. Mirrors the TUI wiring so the pipeline
 * behaves identically across surfaces — the skill just picks up whichever
 * Claude credentials the TUI would have used.
 */
export function buildSkillDeps(opts: BuildOptions): SkillDeps {
...
async function inferDefaultRepo(
  cwd: string,
