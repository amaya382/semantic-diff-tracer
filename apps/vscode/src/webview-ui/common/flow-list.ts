...
export function flowListCss(): string {
  return `
    ...
    .flow-list-row { display: grid; grid-template-columns: 10px 12px 18px minmax(3.5rem, 1fr) auto; align-items: center; gap: 0.25rem; padding: 0.15rem 0.3rem; border-radius: 3px; cursor: pointer; font-size: 0.8rem; line-height: 1.35; color: var(--vscode-foreground); border-left: 3px solid var(--role-accent); }
    ...
    /* Mock chips: one codicon per kind, muted tone so they read as ancillary.
       Glyphs (replace / pin / circle-slash) carry the kind; the three tones
       are near-identical on purpose. Skip drops an extra step because an
       untaken path deserves the least ink. */
    .flow-list-chip.mock-stub { background: var(--vscode-descriptionForeground, #a0a0a0); color: var(--vscode-editor-background); }
    .flow-list-chip.mock-value { background: var(--vscode-descriptionForeground, #a0a0a0); color: var(--vscode-editor-background); opacity: 0.85; }
    .flow-list-chip.mock-skip { background: var(--vscode-descriptionForeground, #a0a0a0); color: var(--vscode-editor-background); opacity: 0.65; }
    /* Focal chips: "this is where the change lives" marker. Core owns the
       loud warm accent so the reader's eye lands there first; entry and
       contract get cooler hues so the three kinds stay separable but do not
       compete with core. Icon-only in the Flow list — the label lives in the
       tooltip and in the Focal card. */
    .flow-list-chip.focal-core { background: var(--vscode-charts-red, #f14c4c); color: #fff; }
    .flow-list-chip.focal-entry { background: var(--vscode-charts-purple, #b180d7); color: #fff; }
    .flow-list-chip.focal-contract { background: var(--vscode-charts-yellow, #d7ba7d); color: var(--vscode-editor-background); }
  `;
}
